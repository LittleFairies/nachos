...
//		System.out.println(currentThread.getName() + " Finished");
		...
		if (currentThread.toBeJoined != null){
			currentThread.toBeJoined.ready();
			currentThread.toBeJoined = null;
		}
...
//		System.out.println(currentThread.getName() + "yielding");
...
//		System.out.println("Calling nextthread: " + currentThread.getName());
		runNextThread();
	}
	...
	public void join() {
		...
		this.toBeJoined = currentThread;

		boolean intStatus = Machine.interrupt().disable();

		this.toBeJoined.sleep();
	...
	public static void selfTest() {
		...
		*/
		...
		KThread C = new KThread(new Runnable() {
			public void run(){
				...
				while(done == 0)
					cv.sleep();
				System.out.println("This is tread C.");
				System.out.println("This is tread C.");
				System.out.println("This is tread C.");
				lock.release();
			}
		...
		KThread D = new KThread(new Runnable() {
			public void run(){
				...
				while(done == 0)
					cv.sleep();
				System.out.println("This is tread D.");
				System.out.println("This is tread D.");
				System.out.println("This is tread D.");
				lock.release();
			}
		...
		KThread B = new KThread(new Runnable() {
			public void run(){
				lock.acquire();
				System.out.println("This is tread B.");
				System.out.println("This is tread B.");
				System.out.println("This is tread B.");
				while (done == 0)
					done = 1;
		...
		*/
		...
		KThread E = new KThread(new Runnable(){
			public void run(){
				...
				System.out.println(Machine.timer().getTime() + "This is Thread E");
		...
		KThread F = new KThread(new Runnable(){
			public void run(){
				System.out.println(Machine.timer().getTime());
				alarm.waitUntil(500);
				System.out.println(Machine.timer().getTime() + "This is Thread F");
			}
		});

		KThread G = new KThread(new Runnable(){
			public void run(){
				System.out.println(Machine.timer().getTime());
				alarm.waitUntil(10);
				System.out.println(Machine.timer().getTime() + "This is Thread G");
			}
		});
		...
		*/
		...
		KThread H = new KThread(new Runnable(){
			public void run(){
//				alarm.waitUntil(1000);
				comm.speak(111);
			}
		...
		KThread I = new KThread(new Runnable(){
			public void run(){
				alarm.waitUntil(1000);
				int word = comm.listen();
				System.out.println(Machine.timer().getTime() + ": "+word);
			}
		});

		KThread J = new KThread(new Runnable(){
			public void run(){
				alarm.waitUntil(1000);
				int word = comm.listen();
				System.out.println(Machine.timer().getTime() + ": "+word);
		...
		KThread L = new KThread(new Runnable(){
			public void run(){
...
//	private static Condition2 cv = new Condition2(lock);
