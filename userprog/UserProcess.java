...
public class UserProcess {
	...
	public int readVirtualMemory(int vaddr, byte[] data, int offset, int length) {
		...
		int cLen = length;
		int cOffset = offset;
		int cPageOffset = pageOffset;
		int cTransferred = 0;

		while(cTransferred < length){
			if(cPageOffset + cLen <= pageSize){
				System.arraycopy(memory, cAddr, data, cOffset, cLen);
				cTransferred += cLen;
				cOffset += cLen;
				pageTable[cVPN].used = false;
				break;
			}else{
				int canTransfer = pageSize - cPageOffset;
				System.arraycopy(memory, cAddr, data, cOffset, canTransfer);
				cTransferred += canTransfer;
				cLen -= canTransfer;
				cOffset += canTransfer;
				cPageOffset = 0;
				pageTable[cVPN++].used = false;
				if(cVPN >= pageTable.length){
					break;
				}else{
					TranslationEntry cEntry = pageTable[cVPN];
					if (!cEntry.valid)break;
					cEntry.used = true;
					cPPN = cEntry.ppn;
					cAddr = cPPN * pageSize;
				}
	...
	public int writeVirtualMemory(int vaddr, byte[] data, int offset, int length) {
		...
		while(cTransferred < length){
			if(cPageOffset + cLen <= pageSize){
				System.arraycopy(data, cOffset, memory, cAddr, cLen);
				cTransferred += cLen;
				cOffset += cLen;
				pageTable[cVPN].used = false;
				break;
				...
				int canTransfer = pageSize - cPageOffset;
				System.arraycopy(data, cOffset, memory, cAddr, canTransfer);
				cTransferred += canTransfer;
				cLen -= canTransfer;
				cOffset += canTransfer;
				cPageOffset = 0;
				...
				if(cVPN >= pageTable.length){
					...
					TranslationEntry cEntry = pageTable[cVPN];
					if (!cEntry.valid || cEntry.readOnly)break;
					cEntry.used = true;
					cPPN = cEntry.ppn;
					cAddr = cPPN * pageSize;
				}
	...
	protected boolean loadSections() {
		...
		for (int s = 0; s < coff.getNumSections(); s++) {
			...
			for (int i = 0; i < section.getLength(); i++) {
				...
				UserKernel.pPagesMutex.P();

				Integer fstPage = null;
				try
			    {
			    	fstPage = UserKernel.pPages.removeFirst();
		...
		for (int i = numPages-9; i < numPages; i++) {
		    ...
		    try
		    {
		    	page = UserKernel.pPages.removeFirst();
	...
	protected void unloadSections() {
		...
		for (int i = 0; i < pageTable.length; i++) {
		    ...
		    if (entry.valid) {
		    	UserKernel.pPagesMutex.P();
		    	UserKernel.pPages.add(entry.ppn);
		    	UserKernel.pPagesMutex.V();
		    }
		}
...
//		Lib.debug(dbgProcess, "Exec: File name " + fileName);
	...
	private int handleJoin(int pid, int status){
		...
		Lib.debug(dbgProcess, "Join: child status, " + childExitStatus);

		if (writeVirtualMemory(status, Lib.bytesFromInt(childExitStatus), 0, 4) < 0){
...
//			child.lock.release();
		...
		Lib.debug(dbgProcess, "Exit: Current pCount: " + pCount);
		if (pCount == 0){
			Kernel.kernel.terminate();
		}
		UserKernel.pCountMutex.V();
